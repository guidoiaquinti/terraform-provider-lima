...
func (d *instanceDataSource) Schema(ctx context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		Attributes: instanceLookupAttributes(ctx, map[string]schema.Attribute{
...
// instanceObservedAttributes returns the read-only attributes describing one
// Lima instance.
//
// Shared by the lima_instance and lima_instances data sources. Both present the
// same view of an instance, and writing the list twice is how the two would
// quietly diverge — TestInstanceDataSourcesAgreeOnAttributes pins that they do
// not.
func instanceObservedAttributes() map[string]schema.Attribute {
	return map[string]schema.Attribute{
		// Computed here because an entry in a list is an observation. The
		// single-instance data source overrides it to Required, where it is the
		// lookup argument instead.
		"name": schema.StringAttribute{
			Computed:            true,
			MarkdownDescription: "The Lima instance name, exactly as `limactl list` reports it.",
		},
...
// instanceLookupAttributes merges the observed attributes with the ones specific
